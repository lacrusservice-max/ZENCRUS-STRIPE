  ...
  return (
    <Screen>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.content}>
        ...
        {/* ── HERO ÚNICO: Score + Anillos fusionados (antes eran 2 heroes redundantes) ── */}
        <View style={s.sec}>
          <View style={s.heroWrap}>
            <LinearGradient
              colors={['rgba(37,99,235,0.24)', 'rgba(0,194,192,0.06)', 'transparent']}
              start={{ x: 0.1, y: 0 }}
              end={{ x: 1, y: 1 }}
            ...
            <View style={s.heroTop}>
              ...
              <View style={s.heroInfo}>
                <Text style={s.heroBrand}>TU SCORE DE HOY</Text>
                <Text style={s.heroMsg}>{getStreakMessage()}</Text>
                <View style={s.heroMetaRow}>
                  <View style={s.heroMetaChip}>
                    <Ionicons name="flame" size={11} color={Colors.accent.orange} />
                    <Text style={s.heroMetaTxt}>{currentStreak}d racha</Text>
                  </View>
                  <View style={[s.heroMetaChip, workedOut && s.heroMetaChipOn]}>
                    <Ionicons name={workedOut ? 'checkmark' : 'barbell-outline'} size={11} color={workedOut ? Colors.accent.green : 'rgba(255,255,255,0.4)'} />
                    <Text style={s.heroMetaTxt}>{workedOut ? 'Entrenado' : 'Sin entreno'}</Text>
                  </View>
                </View>
              </View>
            </View>
...
const s = StyleSheet.create({
  ...
  heroRingsRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  // Primary actions
  ...
  primaryBtnTxtFilled: { fontSize: Typography.fontSize.sm, fontWeight: '800', color: '#fff' },
