...
export default function SocialPage() {
  ...
  return (
    <div style={{ maxWidth: 640, margin: "0 auto", padding: "0 4px 80px" }}>
      <h1 style={{ fontSize: 28, fontWeight: 900, color: C.text, letterSpacing: "-0.03em", padding: "24px 0 16px" }}>Comunidad</h1>

      <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
        {(["all", "following"] as const).map(sc => (
          <button key={sc} onClick={() => setScope(sc)} style={{
            padding: "8px 18px", borderRadius: 999, cursor: "pointer", fontSize: 13.5, fontWeight: 700,
            background: scope === sc ? C.navySoft : C.panel, color: scope === sc ? C.blue2 : C.dim,
            border: `1px solid ${scope === sc ? C.navyBorder : C.border}`,
          }}>
      ...
      <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 16, padding: 16, marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 10 }}>
          <button onClick={publish} disabled={!draft.trim() || posting} style={{
            display: "inline-flex", alignItems: "center", gap: 7, background: draft.trim() ? C.navy : C.panel2,
            color: draft.trim() ? "#fff" : C.dim2, border: "none", borderRadius: 10, padding: "9px 18px",
            fontSize: 13.5, fontWeight: 800, cursor: draft.trim() ? "pointer" : "default",
          }}>
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {posts.map(p => {
            ...
            return (
              <div key={p.id} style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18 }}>
                ...
                {p.content && <div style={{ fontSize: 15, color: C.text, lineHeight: 1.55, marginBottom: 12, whiteSpace: "pre-wrap" }}>{p.content}</div>}

                {(streak || achievement) && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 12 }}>
                    {streak ? <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: C.panel2, borderRadius: 999, padding: "5px 12px", fontSize: 12.5, fontWeight: 700, color: C.orange }}><Flame size={13} /> Racha de {streak} días</span> : null}
                    {achievement ? <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: C.panel2, borderRadius: 999, padding: "5px 12px", fontSize: 12.5, fontWeight: 700, color: C.amber }}><Trophy size={13} /> {achievement}</span> : null}
                  </div>
                )}
