...
/** Fuzzy-match simple contra el catálogo local — usado si falla la llamada al backend. */
function localParseFoodList(mealsText: MealTextInput): ParsedFoodsByMeal {
  ...
  for (const [mealId, text] of Object.entries(mealsText)) {
    ...
    out[mealId] = text.split("\n").map((l) => l.trim()).filter(Boolean).map((line) => {
      ...
      const match = FOOD_DB.find((f) => lower.includes(f.name.toLowerCase()));
      if (match) {
        return {
          name: match.name,
          amount: match.amount,
...
export async function parseFoodList(mealsText: MealTextInput): Promise<ParsedFoodsByMeal> {
  try {
    const { data } = await diet.parseFoodList(mealsText);
