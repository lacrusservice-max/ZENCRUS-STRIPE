...
export interface Recipe {
  ...
  prepTimeMin: number; cookTimeMin: number; servings: number;
  difficulty: "fácil" | "medio" | "difícil";
  ingredients: RecipeIngredient[]; steps: string[]; tags: string[];
  nutrition: { calories: number; protein: number; carbs: number; fat: number };
  isAIGenerated: boolean; isFavorite: boolean; createdAt: string;
  authorId?: string; authorName?: string; isPublic: boolean; imageUri?: string;
  allergens: string[];
}

export interface GroceryItem { name: string; amount: number; unit: string; checked: boolean; }

const DEMO_RECIPES: Recipe[] = [
  {
    id: "r1", title: "Bowl de proteína tropical",
    description: "Desayuno rico en proteínas con frutas tropicales, ideal para post-entreno.",
    emoji: "🥣", category: "desayuno", prepTimeMin: 5, cookTimeMin: 0, servings: 1, difficulty: "fácil",
    ingredients: [
    ...
    isAIGenerated: true, isFavorite: true, createdAt: new Date(Date.now() - 86400000).toISOString(), isPublic: true, allergens: ["lácteos"],
  },
  {
    id: "r2", title: "Ensalada de pollo con aguacate",
    description: "Almuerzo completo con grasas saludables y proteína magra.",
    emoji: "🥗", category: "almuerzo", prepTimeMin: 15, cookTimeMin: 20, servings: 1, difficulty: "fácil",
    ingredients: [
    ...
    isAIGenerated: false, isFavorite: false, createdAt: new Date(Date.now() - 172800000).toISOString(), isPublic: true, allergens: [],
  },
  {
    id: "r3", title: "Avena nocturna con chía",
    ...
    isAIGenerated: false, isFavorite: false, createdAt: new Date(Date.now() - 259200000).toISOString(), isPublic: true, allergens: ["gluten", "frutos-secos"],
  },
];

interface RecipesState {
  recipes: Recipe[]; myRecipes: string[]; favorites: string[];
  allergens: string[]; intolerances: string[];
  addRecipe: (recipe: Omit<Recipe, "id" | "createdAt">) => Recipe;
  deleteRecipe: (id: string) => void;
  toggleFavorite: (id: string) => void;
  setAllergens: (a: string[]) => void;
  setIntolerances: (a: string[]) => void;
...
export const useRecipesStore = create<RecipesState>()(
  persist(
    (set, get) => ({
      recipes: DEMO_RECIPES, myRecipes: [], favorites: ["r1"], allergens: [], intolerances: [],
      addRecipe: (recipeData) => {
        const recipe: Recipe = { ...recipeData, id: `r_${Date.now()}`, createdAt: new Date().toISOString() };
        set((s) => ({ recipes: [recipe, ...s.recipes], myRecipes: [recipe.id, ...s.myRecipes] }));
        return recipe;
      },
      deleteRecipe: (id) => set((s) => ({ recipes: s.recipes.filter((r) => r.id !== id), myRecipes: s.myRecipes.filter((r) => r !== id), favorites: s.favorites.filter((f) => f !== id) })),
      ...
      getFiltered: (search, cat) => {
        ...
        if (search) {
          ...
          results = results.filter((r) => r.title.toLowerCase().includes(q) || r.description.toLowerCase().includes(q) || r.tags.some((t) => t.includes(q)));
        }
        return results;
      },
      getSafe: () => {
      ...
      generateGroceryList: (recipeIds, servings = {}) => {
        ...
        recipeIds.forEach((rid) => {
          ...
          recipe.ingredients.forEach((ing) => {
            ...
            if (map.has(key)) map.get(key)!.amount += Math.round(ing.amount * mult);
            else map.set(key, { name: ing.name, amount: Math.round(ing.amount * mult), unit: ing.unit, checked: false });
          });
        });
        return Array.from(map.values());
      },
    }),
    { name: "zencrus-recipes", storage: createJSONStorage(() => (typeof window !== "undefined" ? localStorage : (undefined as unknown as Storage))) }
  )
);
